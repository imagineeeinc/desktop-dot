#!/usr/bin/env bash
# Configuration
WALLPAPER_DIR="$HOME/Pictures/wallpaper"  # Change this to your wallpaper directory
CACHE_DIR="$HOME/.cache/wallpaper-selector"
THUMBNAIL_WIDTH="320"  # Size of thumbnails in pixels (16:9)
...
selected=$(generate_menu | wofi --show dmenu \
    ...
    --style ~/.config/wofi/wallpaper-style.css \
  )
...
if [ -n "$selected" ]; then
    ...
    if [ -n "$original_path" ]; then
        ...
        awww img "$original_path" --transition-type=outer --transition-pos=bottom --transition-duration=2 --transition-fps=120 --transition-bezier=0.80,0,0.23,1 --resize=crop
